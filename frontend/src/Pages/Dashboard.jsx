...
const Dashboard = () => {
  ...
  const handleSearch = () => {
    ...
    const filteredData = Object.entries(data) // Convert the object to an array of [key, value] pairs
      .map(([key, value]) => {
        // Destructure the key and value
        // Check if any of the properties match the searchText
        if (
          value.title.toLowerCase().includes(searchText.toLowerCase()) ||
          value.description.toLowerCase().includes(searchText.toLowerCase()) ||
          value.dealer.toLowerCase().includes(searchText.toLowerCase()) ||
          value.type.toLowerCase().includes(searchText.toLowerCase()) ||
          value.model === Number(searchText)
        ) {
          return value; // Return the value (car) if it matches
        }
        return null; // Return null if it doesn't match
      })
      .filter((car) => car !== null); // Remove null values from the array
        ...
        <div className="flex gap-4">
          <div className="shadow-lg h-80 w-[500px] rounded-lg bg-white p-12 flex flex-col items-center gap-2">
            ...
            <div className="flex gap-2">
              <TextField
                label="Car Title"
                onChange={(e) => handleUpdateState("title", e.target.value)}
                variant="outlined"
              />
              <TextField
            ...
            <div className="flex gap-2">
              <TextField
                type="number"
                onChange={(e) => handleUpdateState("model", e.target.value)}
                label="Car Model"
                variant="outlined"
              />
              <TextField
                label="Car Dealer"
                onChange={(e) => handleUpdateState("dealer", e.target.value)}
                variant="outlined"
              />
            </div>
            <div className="flex gap-2">
              <TextField
                label="Car Type"
                onChange={(e) => handleUpdateState("type", e.target.value)}
                variant="outlined"
              />
              <TextField
                label="Car Company"
                onChange={(e) => handleUpdateState("company", e.target.value)}
                variant="outlined"
              />
            </div>
        ...
        <div className="flex flex-wrap gap-3 justify-center text-white">
          {filteredCars.length > 0 ? (
            filteredCars.map((car) => (
              <div className="flex  border border-white" key={car.carID}>
                <div className="p-2 bg-black flex flex-col gap-3 justify-start items-center">
                <h3><strong>Title:</strong> {car.title}</h3>
                ...
                <Button onClick={() => navigate(`/Car/carID=${car.carID}`)} variant="contained">Show More</Button>
                </div>
                <div>
                    <img className="h-full w-64" src={car.images[0]} alt="Loading..." />
                </div>
              </div>
            ))
          ) : (
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="overflow-hidden w-96 flex items-center justify-center gap-2">
          <input
            ...
            className="outline-none h-9 p-3 border border-white rounded-md bg-black text-white"
            type="text"
